            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    // done
                    const Eigen::Vector3d ddir(dir.x(), dir.y(), dir.z());
                    float dw = CalcArea(x, y, width, height);
                    int cnt = 0;
                    for (int l = 0; l <= SHOrder; ++l) {
                        for (int m = -l; m <= l; ++m) {
                            double sh_v = sh::EvalSH(l, m, ddir);
                            SHCoeffiecents[cnt++] += Le * sh_v * dw;
                        }
                    }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                // done
                nori::Ray3f ray(v, wi);
                return scene->rayIntersect(ray) ? 0.0 : (H > 0.0 ? H : 0.0);
            };  
        ...
        if (m_Type == Type::Interreflection)
        {
			...
			for (int i = 0; i < mesh->getVertexCount(); i++)
			{
				...
				for (int t = 0; t < sample_side; t++) {
					for (int p = 0; p < sample_side; p++) {
                        ...
                        for (int b = 0; b < m_Bounce; ++b) {
                            ...
                            Intersection inte;
                            nori::Ray3f ray(v, wi);
                            if (wi.dot(n) >= 0.0 && scene->rayIntersect(ray, inte)) {
                                Normal3f t_normal = mesh->getVertexNormals().col(inte.tri_index.x()) * inte.bary.x()
                                    + mesh->getVertexNormals().col(inte.tri_index.y()) * inte.bary.y()
                                    + mesh->getVertexNormals().col(inte.tri_index.z()) * inte.bary.z();
                                double H = -wi.dot(t_normal.normalized());

                                if (H > 0.0) {
                                    for (int j = 0; j < SHCoeffLength; ++j) {
                                        coeffs[j] += (m_TransportSHCoeffs.col(inte.tri_index.x()).coeffRef(j) * inte.bary.x()
                                            + m_TransportSHCoeffs.col(inte.tri_index.y()).coeffRef(j) * inte.bary.y()
                                            + m_TransportSHCoeffs.col(inte.tri_index.z()).coeffRef(j) * inte.bary.z()
                                            ) * H / (2.0 * M_PI);
                                    }
                                }
                ...
                for (unsigned int j = 0; j < SHCoeffLength; ++j) {
                    coeffs[j] *= weight;
                    m_TransportSHCoeffs.col(i).coeffRef(j) += coeffs[j];
                    //printf("%f ", coeffs[j]);
                }
                //printf("\n");
