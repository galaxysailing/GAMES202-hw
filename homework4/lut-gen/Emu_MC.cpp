...
float GeometrySchlickGGX(float NdotV, float roughness) {
    ...
    //float denominator = std::max((NdotV * (1.0 - k) + k), 0.00001);
...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      ...
      float NdotL = dot(N, sampleList.directions[i]);
      ...
      //float denominator = std::max((4.0 * NdotL * NdotV), 0.0001);
      fr = fr / denominator;
      float cosTheta = sampleList.directions[i].z;

      //float sinTheta = sqrt(std::max(1.0f - sampleList.directions[i].z * sampleList.directions[i].z, 0.0f));
      A += fr.x * cosTheta / sampleList.PDFs[i];
      B += fr.y * cosTheta / sampleList.PDFs[i];
      C += fr.z * sampleList.directions[i].z / sampleList.PDFs[i];
